...
// NewObjectKey 生成对象键，按课程和章节分层，便于管理资源。
func (s *Service) NewObjectKey(courseID, chapterID, filename string) string {
	return fmt.Sprintf("courses/%s/chapters/%s/%s-%s", courseID, chapterID, uuid.NewString(), filename)
}
