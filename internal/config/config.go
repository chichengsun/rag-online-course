...
// CourseDesignConfig 教师端「课程设计」中 AI 生成教学大纲的提示词等（与 RAG 问答提示词分离）。
type CourseDesignConfig struct {
	// OutlineSystemPrompt 生成大纲时发给模型的 system 角色内容。
	OutlineSystemPrompt string `mapstructure:"outline_system_prompt"`
	// OutlineUserPromptTemplate 生成大纲时 user 角色模板；Go text/template，字段：Title、Description、ExtraHint（ExtraHint 为空则 if 块不输出）。
	OutlineUserPromptTemplate string `mapstructure:"outline_user_prompt_template"`
	// LessonPlanSystemPrompt 生成小节教案时发给模型的 system 角色内容。
	LessonPlanSystemPrompt string `mapstructure:"lesson_plan_system_prompt"`
	// LessonPlanUserPromptTemplate 生成小节教案时 user 模板；字段：CourseTitle、ChapterTitle、SectionTitle、Objectives、TeachingStyle、DurationMinutes、ExtraHint、Resources。
	LessonPlanUserPromptTemplate string `mapstructure:"lesson_plan_user_prompt_template"`
}

// QuestionBankConfig 课程题库导入配置：上传文本经 AI 解析为题目列表。
type QuestionBankConfig struct {
	// ImportSystemPrompt 题库导入时的 system 角色提示词。
	ImportSystemPrompt string `mapstructure:"import_system_prompt"`
	// ImportUserPromptTemplate 题库导入 user 提示模板；字段：CourseTitle、FileName、Content。
	ImportUserPromptTemplate string `mapstructure:"import_user_prompt_template"`
}
...
type Config struct {
	...
	// CourseDesign 课程设计（AI 大纲草案）提示词配置。
	CourseDesign CourseDesignConfig `mapstructure:"course_design"`
	// QuestionBank 课程题库（文件解析导题）提示词配置。
...
- 保留专有名词、英文缩写、数字版本信息。`)
	...
	v.SetDefault("course_design.outline_system_prompt", strings.TrimSpace(`你是课程结构设计助手。只输出一个 JSON 对象，不要 Markdown 代码块、不要解释文字。
JSON 顶层键为 "chapters"，值为数组；每项含 "title"（章标题）与 "sections"（数组，元素为 {"title":"节标题"}）。
章节数量建议 5～10，每章 2～6 节；标题简洁、中文；须贴合给定课程主题。`))
	v.SetDefault("course_design.outline_user_prompt_template", strings.TrimSpace(`课程标题：{{ .Title }}
课程描述：{{ .Description }}
{{- if .ExtraHint }}
...
请生成教学大纲 JSON。`))
	v.SetDefault("course_design.lesson_plan_system_prompt", strings.TrimSpace(`你是高校教师助教，请基于给定的小节目标与资源上下文，生成可执行的课堂教案。
...
约束：不得编造资源中不存在的事实；steps 至少 3 个；内容必须是中文且紧贴小节主题。`))
	v.SetDefault("course_design.lesson_plan_user_prompt_template", strings.TrimSpace(`课程：{{ .CourseTitle }}
章节：{{ .ChapterTitle }}
小节：{{ .SectionTitle }}
教学目标：
{{- range .Objectives }}
...
请按 system 约束输出教案 JSON。`))
	v.SetDefault("question_bank.import_system_prompt", strings.TrimSpace(`你是课程题库整理助手。请根据输入文本抽取可用于教学测评的题目。
你必须只输出一个 JSON 对象，不要 Markdown 代码块、不要解释。
JSON 格式：
{
  "questions": [
...
- 输出 3~20 道高质量题目；若原文信息不足，可少量输出但不能编造无关事实。
- 题目语言使用中文，紧贴课程文本内容。`))
	v.SetDefault("question_bank.import_user_prompt_template", strings.TrimSpace(`课程：{{ .CourseTitle }}
文件名：{{ .FileName }}
