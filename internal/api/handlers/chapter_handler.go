...
// UpdateChapter PUT /teacher/chapters/:chapterId — 更新章节标题与排序。
func (h *ChapterHandler) UpdateChapter(c *gin.Context) {
	...
	if err := h.chapterSvc.UpdateChapter(c.Request.Context(), req.ChapterID, req.TeacherID, req.Title, req.SortOrder); err != nil {
		if err == postgres.ErrNotFound {
			response.Error(c, http.StatusNotFound, "update chapter failed")
			return
		}
		if respondPostgresUniqueViolation(c, err, "update_chapter") {
			return
		}
		response.Error(c, http.StatusInternalServerError, "update chapter failed")
		return
	}
	c.Status(http.StatusNoContent)
}
