...
func NewRouter(userH *handlers.UserHandler, courseH *handlers.CourseHandler, chapterH *handlers.ChapterHandler, resourceH *handlers.ResourceHandler, enrollmentH *handlers.EnrollmentHandler, catalogH *handlers.CatalogHandler, progressH *handlers.ProgressHandler, knowledgeH *handlers.KnowledgeHandler, aiModelH *handlers.AIModelHandler, knowledgeChatH *handlers.KnowledgeChatHandler, jwtSvc *auth.JWTService, sessionStore *auth.SessionStore) *gin.Engine {
	r := gin.New()
	r.Use(
		middleware.CORS(),
	...
	v1 := r.Group("/api/v1")
	{
		...
		teacher.Use(middleware.RequireAuth(jwtSvc, sessionStore), middleware.RequireRole("teacher"))
		{
			...
			teacher.GET("/chapters/:chapterId/resources", resourceH.ListResources)
			teacher.POST("/chapters/:chapterId/resources/init-upload", resourceH.InitUpload)
			teacher.POST("/chapters/:chapterId/resources/confirm", resourceH.ConfirmResource)
			teacher.GET("/resources/:resourceId/preview-url", resourceH.PreviewResourceURL)
