...
func userMessageForUniqueViolation(constraint, table string) string {
	...
	case "uk_course_sections_chapter_title":
		return "该章节下已存在同名节，请修改节标题后重试"
	case "uk_chapter_resources_section_title":
		return "该节下已存在同名资源，请修改资源标题（可与文件名区分）后重试"

	// —— 表内 UNIQUE，PostgreSQL 默认约束名 ——
	case "course_chapters_course_id_sort_order_key":
		return "该课程下章节排序号与已有章节冲突，请更换排序后重试或刷新页面"
	case "course_sections_chapter_id_sort_order_key":
		return "该章节下节的排序号已存在，请修改排序后重试或刷新页面"
	case "chapter_resources_chapter_id_sort_order_key":
		return "该章节下资源排序号已存在，请修改排序后重试或刷新页面"
	case "uk_chapter_resources_section_sort":
		return "该节下资源排序号已存在，请修改排序后重试或刷新页面"
