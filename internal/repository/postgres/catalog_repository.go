...
func (r *CatalogRepository) GetCatalog(ctx context.Context, courseID, studentID int64) (map[string]any, error) {
	...
	for _, chapterRow := range chapterRows {
		...
		sectionRows := make([]map[string]any, 0)
		secErr := r.db.WithContext(ctx).Raw(`
			SELECT id::text AS id, title, sort_order
			FROM course_sections
			WHERE chapter_id = $1
			ORDER BY sort_order ASC, id ASC
		`, chapterIDStr).Scan(&sectionRows).Error
		if secErr != nil {
			return nil, secErr
		}
		for _, sectionRow := range sectionRows {
			sectionIDStr, secConvErr := scalarIDToString(sectionRow["id"])
			if secConvErr != nil || sectionIDStr == "" {
				return nil, ErrInvalidID
			}
			resourceRows := make([]map[string]any, 0)
			resourceScanErr := r.db.WithContext(ctx).Raw(`
				SELECT id::text AS id, course_id::text AS course_id, chapter_id::text AS chapter_id,
				       section_id::text AS section_id, title, resource_type::text AS resource_type,
				       sort_order, object_url, mime_type, size_bytes,
				       ai_summary, ai_summary_updated_at
				FROM chapter_resources
				WHERE section_id = $1
				ORDER BY sort_order ASC, id ASC
			`, sectionIDStr).Scan(&resourceRows).Error
			if resourceScanErr != nil {
				return nil, resourceScanErr
			}
			sectionRow["resources"] = resourceRows
		}
		chapterRow["sections"] = sectionRows
	}
